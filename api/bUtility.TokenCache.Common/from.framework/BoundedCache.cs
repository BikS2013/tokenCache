...
namespace bUtility.TokenCache
{
    ...
    {
        readonly Dictionary<string, ExpirableItem<T>> _items;
        int _capacity;
        TimeSpan _purgeInterval;
        readonly ReaderWriterLock _readWriteLock;
        ...
        public BoundedCache(int capacity, TimeSpan purgeInterval, IEqualityComparer<string> keyComparer)
        {
            if (capacity <= 0)
            {
                throw new ArgumentOutOfRangeException("capacity", capacity, "Only positive values are valid");
            }

            if (purgeInterval <= TimeSpan.Zero)
            {
                throw new ArgumentOutOfRangeException("purgeInterval", purgeInterval, "Only positive values are valid");
            }
        ...
        public virtual int Capacity
        {
            ...
            set
            {
                if (value <= 0)
                {
                    throw new ArgumentOutOfRangeException("value", value, "Only positive values are valid");
                }
                _capacity = value;
            }
        }
        ...
        void EnforceQuota()
        {
            ...
            {
                throw new LimitExceededException("Exceeded capacity " + _capacity);
            }
        }
        ...
        public virtual int IncreaseCapacity(int size)
        {
            if (size <= 0)
            {
                throw new ArgumentOutOfRangeException("size", size, "Only positive values are valid");
            }
